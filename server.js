...
<div id="wrap">
  ...
  <div id="ver">v3.20 · Paperdoll: thêm TÓC (hết trọc) + giáp đổi KIỂU theo cấp</div>
</div>
<script>
var WW=560, WH=420;
...
function drawPixelChar(px,py,hue,cls,fx,fy,moving,mvx,mvy,isSelf){
  ...
  if(isSelf && PD.ready){
    ...
    if(!(eq&&eq.hlm)) ctx.drawImage(PD.hair,col*64,row*64,64,64,tx,ty,SZp,SZp);
    var _at=(eq&&eq.arm)?(eq.arm.tier||1):0;
    if(_at===0){ ctx.drawImage(PD.tab,col*64,row*64,64,64,tx,ty,SZp,SZp); }
    else if(_at===1){ drawArmorLayer(PD.mail,col,row,rarityFX(eq.arm),tx,ty,SZp); }
    else if(_at===2){ drawArmorLayer(PD.mail,col,row,{tint:null,glow:0},tx,ty,SZp); ctx.drawImage(PD.tab,col*64,row*64,64,64,tx,ty,SZp,SZp); }
    else { drawArmorLayer(PD.mail,col,row,rarityFX(eq.arm),tx,ty,SZp); drawArmorLayer(PD.tab,col,row,rarityFX(eq.arm),tx,ty,SZp); }
...
dbInit();
server.listen(PORT,()=>console.log('✅ WEBGAME v3.20 (tóc + giáp đổi kiểu theo cấp) chạy ở cổng '+PORT));
