...
<div id="wrap">
  ...
  <div id="ver">v3.07 · NHÂN VẬT — sprite LPC 4 hướng + animation đi bộ</div>
</div>
<script>
var WW=560, WH=420;
...
function drawPixelChar(px,py,hue,cls,fx,fy,moving,mvx,mvy){
  var PS=3.9;
...
function frame(now){
 try{
  ...
  for(var id in players){var p=players[id]; if(p.zone!==myZone)continue;
    ...
    drawPixelChar(rx,ry,p.hue,p.cls,ffx,ffy,(p._prx!==undefined&&(Math.abs(rx-p._prx)>0.15||Math.abs(ry-p._pry)>0.15)),_mvx,_mvy);
    p._prx=rx; p._pry=ry;
...
dbInit();
server.listen(PORT,()=>console.log('✅ WEBGAME v3.07 (LPC nhân vật 4 hướng + walk animation) chạy ở cổng '+PORT));
