...
<div id="wrap">
  ...
  <div id="ver">v3.13 · Map cũ đủ 5 loại quái/map (thật hết)</div>
</div>
<script>
var WW=560, WH=420;
...
function drawPixelChar(px,py,hue,cls,fx,fy,moving,mvx,mvy,isSelf){
  ...
  if(isSelf && PD.ready){
    ...
    if(eq&&eq.hlm) drawArmorLayer(PD.helm,col,row,rarityFX(eq.hlm),tx,ty,SZp);
    return;
  }
 ...
 try{
  ...
  for(var eid in enemies){var en=enemies[eid];if(en.dead||en.zone!==myZone)continue;var er=en.r||14;
    ...
    if(en.boss){
      ...
      if(en.zone==='swamp' && mob2Ready){ drawMob2('turtle',en.x,en.y-er*0.3,er*3.4); }
    ...
    else if(msh && mob2Ready && MOB2[msh]){ drawMob2(msh,en.x,en.y-Math.abs(Math.sin(performance.now()/480+en.x*0.4))*1.4,er*2.7); }
    else if(msh && mob3Ready && MOB3[msh]){ drawMob3(msh,en.x,en.y-Math.abs(Math.sin(performance.now()/480+en.x*0.4))*1.4,er*2.7); }
...
const MOB_TYPES = {
  ...
  cave: [
    ...
    {id:'wraith', name:'Ma Sương', hue:220, hpMul:1.0, spdMul:1.0, dmgMul:1.2, shape:'wraith'},
    {id:'cskel', name:'Xương Băng', hue:200, hpMul:1.1, spdMul:0.95, dmgMul:1.1, shape:'skeleton'},
  ...
  dungeon: [
    ...
    {id:'dwraith', name:'Oán Linh', hue:280, hpMul:1.1, spdMul:1.0, dmgMul:1.25, shape:'wraith'},
    {id:'dimp', name:'Tiểu Ma', hue:330, hpMul:0.95, spdMul:1.25, dmgMul:1.2, shape:'imp'},
    {id:'dturtle', name:'Giáp Vệ Cổ', hue:60, hpMul:1.7, spdMul:0.5, dmgMul:1.0, shape:'turtle'},
  ],
  swamp: [
...
dbInit();
server.listen(PORT,()=>console.log('✅ WEBGAME v3.13 (map cũ 5 quái/map, thật hết) chạy ở cổng '+PORT));
